...
def offset_image_id(conn: sqlite3.Connection, offset: int):
    cur = conn.cursor()
...
def correct_image_id(
    ...
    try:
        ...
        before_image_id = result[0][0]

        cur.execute(
            "UPDATE images SET image_id = ? WHERE name = ?", (image_id, image_name)
        )

        cur.execute(
            "UPDATE descriptors SET image_id = ? WHERE image_id = ?",
            (image_id, before_image_id),
        )
        cur.execute(
            "UPDATE keypoints SET image_id = ? WHERE image_id = ?",
            (image_id, before_image_id),
        )
        cur.execute(
            "UPDATE pose_priors SET image_id = ? WHERE image_id = ?",
            (image_id, before_image_id),
        )
